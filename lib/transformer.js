...
class Transformer {
    constructor(object, app) {
        this.object = object;
        this.app = app;
        const [newLTPoint, newRTPoint, newLBPoint, newRBPoint, newTPoint, newBPoint, newLPoint, newRPoint, newUBPoint] = object.points;
    ...
    static bindShotcuts(app) {
        ...
        keyboardjs.bind("ctrl + shift + g", e => {
            ...
            compose.children.forEach(_ => {
                app.stage.addChild(new Transformer(new Component(getParams(_, compose)), app).object);
            });
            compose.selection.destroy();
            compose.destroy();
            app.stage.removeChild(compose.selection);
